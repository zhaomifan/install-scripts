#!/bin/bash
# ==================================================
# RocketMQ 5.4.0 集群版部署脚本（增强版）
# 修复点：停止服务增加容错 || true，解决重启循环
# 新增功能：
# 1. 用户认证配置（生产者/消费者需用户名密码）
# 2. 禁用自动创建Topic
# 3. 自动生成 acl.yml 权限控制文件
# ==================================================
...
# 4.0 生成 ACL 权限配置文件 (acl.yml)
# 重点：配置全局白名单，允许内网管理工具连接
echo "=== 生成 ACL 权限配置 (acl.yml) ==="
cat > ${INSTALL_DIR}/conf/acl.yml <<EOF
